const fs = require('fs');
const filepath = 'src/app/assistente/page.tsx';
let content = fs.readFileSync(filepath, 'utf-8');

// 1. Replace Footer Texts
content = content.replace(/Comunidade Corações Sagrados❤️🔥/g, "Projeto Corações Sagrados❤️‍🔥");
content = content.replace(/Comunidade Corações Sagrados/g, "Projeto Corações Sagrados");
content = content.replace(/❤️🔥/g, "❤️‍🔥");

// 2. Add copy button to formatador
const copy_all_handler = `  // --- Helper para copiar texto para todos os dias ---
...
`;
content = content.replace(/placeholder=\{\`Cole o texto específico da meditação do dia \$\{i \+ 1\} aqui\.\`\}\n\s+className="w-full bg-stone-900 border border-white\/10 rounded-xl px-3 py-2 text-xs text-stone-100 focus:outline-none focus:border-amber-500 resize-none font-medium leading-relaxed"\n\s+\/>/g, `placeholder={\`Cole o texto específico da meditação do dia \${i + 1} aqui.\`}
...
const textWhatsAppSantoMatch = content.match(/const textWhatsAppSanto = useMemo\(\(\) => \{[\s\S]*?(?=const textInstagramSanto = useMemo)/);
if (textWhatsAppSantoMatch) {
  const santo_wa_replacement = `const textWhatsAppSanto = useMemo(() => {
  ...
  `;
  content = content.replace(textWhatsAppSantoMatch[0], santo_wa_replacement);
}

const getFormattedNovenaTextCompactMatch = content.match(/const getFormattedNovenaTextCompact = \([\s\S]*?_Projeto Corações Sagrados❤️‍🔥_\`;\n\n    return text;\n  \};/);
if (getFormattedNovenaTextCompactMatch) {
  const novena_format_repl = `const getFormattedNovenaTextCompact = (
    id: string,
    dayIdx: number,
  ...
  content = content.replace(getFormattedNovenaTextCompactMatch[0], novena_format_repl);
}
